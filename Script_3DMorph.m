...
for total = 1:numFiles
    %% Load file and saved values
    
    clearvars -except file ch ChannelOfInterest scale zscale Parameters FileList PathList Interactive NoImages total numFiles
    ...
    if Interactive == 2
        ...
        nucLevel = graythresh(img) * nucAdjust;
        NucBinaryThresholdedImg = imbinarize(img, nucLevel);
        NucNoiseIm=bwareaopen(NucBinaryThresholdedImg,nucNoise);
    end
    
    ConnectedNuclei = bwconncomp(NucNoiseIm,26);
    ...
    for i = 1:numObj
        waitbar (i/numObj/2, progbar);
        valid = 0;
        
        for j=1:ConnectedNuclei.NumObjects
            overlap = intersect(ConnectedComponents.PixelIdxList{i}, ConnectedNuclei.PixelIdxList{j});
            if ~isempty(overlap)
                if valid==0
                    valid = 1;
                    validObjects = [validObjects, i];
                    validIndex = validIndex + 1;
                    ObjectNuclei{validIndex} = [j];
                else
                    ObjectNuclei{validIndex} = [ObjectNuclei{validIndex}, j];
                end
            end
        end
    end
    
    numValid = length(validObjects);
    allObjs = zeros(s(1), s(2), numValid);
    ConnectedComponents.NumObjects = numValid;
    ConnectedComponents.PixelIdxList = ConnectedComponents.PixelIdxList(validObjects);
    numObj = numValid;
    
    for i = 1:numObj
        waitbar (0.5 + i/numObj, progbar);
        [x,y,z] = ind2sub([s, zs], ConnectedComponents.PixelIdxList{i});
        [u_xy,~,ib] = unique([x,y], 'rows');
        u_xy(:,3) = i;
        ind = sub2ind([s, numObj],u_xy(:,1), u_xy(:,2), u_xy(:,3));
        sums = hist(ib, unique(ib));
        allObjs(ind) = sums;
    end
    ...
    levels = adjust:(nucAdjust-adjust)/100:nucAdjust;
    ...
    for i = 1:numObj %Evaluate all connected components in PixelIdxList.
        ...
        if length(ObjectNuclei{i}) > 1
            ...
            nuc = length(ObjectNuclei{i});
            if nuc ==0 %If erosion only detects one nuc, but this should be segmented, increase nuc to at least 2
                disp('Error: The program finds 0 nuclei to segement your object into. Skipping...');
                figure('Name',['Failed nucleus search (Object ', num2str(i), ')']); 
                imshow(max(ex,[],3))
                continue
            end
            ...
            nucInd = ObjectNuclei{i};
            centroids = zeros(nuc,3);
            Ds = zeros(size(points,1), nuc);
            
            parfor n = 1:nuc
                nucleus = ConnectedNuclei.PixelIdxList{nucInd(n)};
                [n_x,n_y,n_z] = ind2sub(size(ex), nucleus);
                centroids(n,:) = mean([n_x,n_y,n_z]);
                cent = round(centroids(n,:));
                
                D = bwdistgeodesic(logical(ex), sub2ind(size(ex),cent(1), cent(2), cent(3)), 'quasi-euclidean');
                Ds(:,n) = D(~isnan(D));
            end
    ...
    for i = 1:numObjSep
        [x,y,z] = ind2sub([s, zs], Microglia{1,i});
        [u_xy,~,ib] = unique([x,y], 'rows');
        u_xy(:,3) = i;
        ind = sub2ind([s, numObjSep],u_xy(:,1), u_xy(:,2), u_xy(:,3));
        sums = hist(ib, unique(ib));
        AllSeparatedObjs(ind) = sums;
    end
    if isgraphics(progbar)
        close(progbar);
    end
    ...
    ObjectList = SepObjectList;%Sort columns by pixel size.
    ...
    if KeepAllCells == 1
        ...
        for i = 1:numObjSep
            waitbar (i/numObjSep, progbar);
            if numel(Microglia{1,i})>=SmCellCutoff && numel(Microglia{1,i}) <= CellSizeCutoff %Extract elements of Microglia that are >SmCellCutoff pixels
                if RemoveXY ==1
                    ex=zeros(s(1),s(2),zs);%Create blank image of correct size
    ...
    for i=1:numObjMg
        ...
        for j=1:ConnectedNuclei.NumObjects
            overlap = intersect(FullMg{i}, ConnectedNuclei.PixelIdxList{j});
            if ~isempty(overlap)
                ObjectNuclei{i} = j;
                break;
            end
        ...
        nucleus = ConnectedNuclei.PixelIdxList{ObjectNuclei{i}};
        [n_x,n_y,n_z] = ind2sub([s, zs], nucleus);
        cent(i,:) = mean([n_y,n_x,n_z]); % Switch x and y because that's how it gets used below?
    end
    ...
    %for i=1:numel(FullMg)
        try
            ...
            if SkelImg == 1
                title = [file,'_Cell',num2str(i)];
                fig = figure('Name',title); %Plot all branches as primary (red), secondary (yellow), tertiary (green), or quaternary (blue).
                hold on
            ...
            endpts2 = (convn(BoundedSkel,kernel,'same')==1)& BoundedSkel;
            quatbrpts = quatendpts - endpts2; %Have points at both ends of final branches. Want to exclude any distal points (true endpoints)
            ...
            if EndImg == 1
                title = [file,'_Cell',num2str(i)];
                fig = figure('Name',title); %Plot all branches with endpoints
            ...
            if BranchImg == 1
                title = [file,'_Cell',num2str(i)];
                fig = figure('Name',title); %Plot all branches with branchpoints
                fv1=isosurface(fullmask,0);%display each object as a surface in 3D. Will automatically add the next object to existing image.
                patch(fv1,'FaceColor',[0 0 1],'FaceAlpha',0.1,'EdgeColor','none');%without edgecolour, will auto fill black, and all objects appear black
                camlight %To add lighting/shading
                lighting gouraud; %Set style of lighting. This allows contours, instead of flat lighting
    ...
    if BranchLengthFile == 1
        names = ["cell1"];
        %Write in headings
        lengths = zeros(numel(FullMg,1));
        for CellNum = 1:numel(FullMg)
            input = strcat('Cell ',num2str(CellNum));
            names(CellNum,1) = input;
            lengths(CellNum) = length(BranchLengthList{CellNum});
        end
        BranchFilename = 'BranchLengths.csv';
        maxlen = max(lengths);
        data = repmat([""], length(names), maxlen+1);
        data(:,1) = names;
        
        %Write in data
        for ColNum = 1:numel(FullMg)
            % TODO get longest list and pad all other lists to that length,
            % then write
            if numel(BranchLengthList{1,ColNum})>0
                data(ColNum,2:lengths(ColNum)+1) = BranchLengthList{ColNum}';
            end
        end
        
        writematrix(data, fullfile(fpath, BranchFilename));
    end
    
    %% Output results
    %Creates new excel sheet with file name and saves to current folder.
    
    ResultsFile = fullfile(fpath, strcat('Results_General_', file, '.csv'));
    
    data = [["Avg Centroid Distance um", "TotMgTerritoryVol um3", ...
             "TotUnoccupiedVol um3", "PercentOccupiedVol um3"]; ...
            [AvgDist, TotMgVol, EmptyVol, PercentMgVol]]; 
            
    writematrix(data, ResultsFile);
    
    ResultsFile = fullfile(fpath, strcat('Results_PerCell_', file, '.csv'));
    headings = ["Cell", "CellTerritoryVol", "CellVolumes", ...
                "RamificationIndex", "NumEndpoints", "NumBranchPoints", ... 
                "AvgBranchLength", "MaxBranchLength", "MinBranchLength", ...
                "CentroidX", "CentroidY", "CentroidZ"];
    
    data = [(1:numel(FullMg))', FullCellTerritoryVol(:,1), ... 
            CellVolume(:,1), FullCellComplexity(:,1), numendpts(:,1), ...
            numbranchpts(:,1), AvgBranchLength(:,1), ...
            MaxBranchLength(:,1), MinBranchLength(:,1), cent];
        
    writematrix([headings; data], ResultsFile);
    
    if Interactive == 2
        disp(['Finished file ' num2str(total) ' of ' num2str(numFiles)]);
    end
...
if Interactive == 1
    time = clock;
    name = ['Parameters_',file,'_',num2str(time(1)),num2str(time(2)),num2str(time(3)),num2str(time(4)), '.mat'];
    save(name,'ch','ChannelOfInterest','scale','zscale','adjust','noise','nucAdjust','nucNoise','s','ShowImg','ShowObjImg','ShowCells','ShowFullCells','CellSizeCutoff','SmCellCutoff','KeepAllCells','RemoveXY','ConvexCellsImage','SkelMethod','SkelImg','OrigCellImg','EndImg','BranchImg','BranchLengthFile');
end
